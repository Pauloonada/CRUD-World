...
interface ModalExcluirPaisProps{
    modalVisible: boolean;
    setModalVisible: (visible: boolean) => void;
    paisExcluido: any;
    handleExcluirPais: (id:number) => void;
}

export default function ModalExcluirPais({
    modalVisible,
    setModalVisible,
    paisExcluido,
    handleExcluirPais,
}: ModalExcluirPaisProps){
            ...
            <View style={styles.modalOverlay}>
                <View style={[styles.modalBox, { backgroundColor: theme.card }]}>
                    ...
                    <Text style={{ color: theme.text, textAlign: "center", fontSize: 16, marginBottom: 20 }}>
                        ...
                        <Text style={{ fontWeight: "bold", color: theme.error }}>{paisExcluido?.nome_oficial || "este país"}?{"\n"}Essa ação não pode ser desfeita.</Text>
                    </Text>

                    <View style={styles.buttonsRow}>
                        <TouchableOpacity style={[styles.button, { backgroundColor: theme.error, flex: 1, marginRight: 8 }]} onPress={() => handleExcluirPais(paisExcluido.id)}>
